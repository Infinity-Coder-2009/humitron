...
class TokenUsage:
    ...
    Attributes:
        ...
        started_at: Session start time (ISO format).
...
class TokenBudgetManager:
    ...
    def __init__(
        ...
        Args:
            ...
            ollama_client: Optional Ollama client for summarization.
        """
    ...
    def _summarize_middle(
        ...
        Returns:
            Messages with middle summarized.
        """
        system_msg = next((m for m in messages if m["role"] == "system"), None)
        other_messages = [m for m in messages if m["role"] != "system"]
...
def get_token_budget_manager(
    ...
    Args:
        max_tokens: Maximum tokens per session.

...
def web_search(query: str, max_results: int = 5) -> ToolResult:
    ...
    Raises:
        ToolResult with error if search fails.
    """
    ...
    try:
        ...
        if not parser.results:
            logger.warning(f"No structured results found for query: {query}")
            return ToolResult(
                success=True,
        ...
        logger.info(f"Found {len(parser.results)} results for query: {query}")
